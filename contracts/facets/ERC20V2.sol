...
contract ERC20V2 {
   ...
   function setValue(string memory _name, string memory _symbol, uint _decimals) public {
    ...
    function setPlatformOwner(address _platformOwner) public {
        erc20State.platformOwner = _platformOwner;
    }

    function __transfer(address _from , address _to, uint _amount ) internal  returns(bool){
        // require(erc20State.platformOwner != address(0), "Platform Owner not set!");
        erc20State.balanceOf[_from] -= _amount;
        erc20State.balanceOf[erc20State.platformOwner] +=  (5*_amount/100);
        erc20State.balanceOf[_to] += (95*_amount/100);
